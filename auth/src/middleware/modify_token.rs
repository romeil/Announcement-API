        ...
        Box::pin(async move {
            ...
            if alter_token {
                ...
                if cookie_header_vals.len() > 0 {
                    match path {
                        "/login/admin" | "/login/club" | "/login/prefect" | "/register" | "/create-pin" => {    
...
pub fn alter_csrf_token(request: &ServiceRequest) -> bool {
    if request.path() == "/register" || request.path() == "/create-pin" || request.path() == "/login/admin" || request.path() == "/login/club" || request.path() == "/login/prefect" ||  request.path() == "/logout"|| (request.path() == "/club" && request.method().as_str() == "POST") {
        true
    } else {
        false
    }
